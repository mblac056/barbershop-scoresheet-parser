...
if __name__ == "__main__":
    ...
    base_path = args.input.rsplit('.', 1)[0]  # Remove .pdf extension
    csv_path = f"{base_path}.csv"
    json_path = f"{base_path}.json"
    pivot_csv_path = f"{base_path}_pivot.csv"

    # Extract tables from the PDF and save as a CSV
    extract_tables(args.input, csv_path)

    # Parse the CSV and save as a JSON
    parse_table(csv_path, json_path)

    # Create pivot table format
    create_pivot_format(json_path, pivot_csv_path)
